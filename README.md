...
```bash
python3 skills/audit-skill-supply-chain/scripts/safe_install_skill.py \
  --artifact /path/to/release.zip \
  --expected-sha256 <sha256> \
  --cli both
```
...
6. **Promote exact artifact only**: if allowed, promote the same private staging copy into the live install path.
...
6. **只提升已审查产物**：如果允许安装，只提升私有暂存中的同一份已审查 skill。
