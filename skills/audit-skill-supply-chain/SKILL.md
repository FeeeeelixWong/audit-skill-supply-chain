...
- For repository enforcement, use the project's reusable GitHub Action pinned to a reviewed full commit SHA; retain JSON evidence with restricted access and short retention, and upload redacted SARIF only with the minimum required GitHub permissions.
...
```bash
report_dir="$(mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/agent-skill-audit.XXXXXX")"
...
SARIF locations are repository-relative when the skill is inside a checkout and target-relative otherwise. SARIF excludes both the scanner host's absolute target path and raw finding evidence; detailed evidence remains in JSON. Treat both reports as untrusted data and do not execute commands quoted from a finding.
